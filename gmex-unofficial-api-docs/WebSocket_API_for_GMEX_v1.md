...
请求参数中，Sec 是K线数据开始的时间(秒)， Count 为多个少，即从开始时间开始需要
多少个K线记录， Offset为偏移。
这里需要注意的是，如果某个时刻没有数据(没有成交)，则其对应的K线是不会有记录的，
因此返回的结果用户需要自己对应其记录的时间戳进行比对。
有时候一个交易对可能会很长时间没有交易，客户端无法方便的判断出需要查询开始k线的
时间，为此特别提供了一个查询最近K线数据的指令 GetLatestKLine 方便使用。
还有个返回值 InitSec 是表明这个交易对从创建开始后第一个分钟kline的时间，单位秒,
查询kline数据时，在这个时间前是一定没有数据的。


获取最近K线数据的指令: GetLatestKLine
注意: 这个指令服务器端对于Count为100倍数的请求有加速(Count最大3000)，推荐大家使用。
```js
// 比如要查询交易对 BTC.USDT 最近200个 [1分钟k] 的数据，请求消息如需：
{"req":"GetLatestKLine","rid":"2","expires":1564109336120,"args":{"Sym":"BTC.USDT","Typ":"1m","Count": 200}}
...
{"rid":"2","code":0,"data":{
    ...
    "Count":200,
    "InitSec":1545152760,
    "Sec":[1582958580,1582958520,....,1582946640],
    "PrzOpen":[8720,8721,...,8699],
    "PrzHigh":[8728,8726,...,8706],
    "PrzLow":[8720,8718.5,...,8699],
    "Volume":[20250,18409,...,8944],
    "Turnover":[883374.24,802779.935,...,389143.63]
    }
}
```
...
5. 批量订阅tick行情（新增）
```js
// 为了优化客户端的网络质量，应某些方面的需求，特此提供批量订阅tick行情的指令
//
// 发送订阅请求的消息，如下格式：
{"req":"SubTicks","rid":"160","expires":1584625796537,"args":["BTC.BTC","BTC.USDT","EOS.USDT","GMEX_CI_BTC","GMEX_CI_EOS"]}

// 收到的回复消息如下：
{"rid":"160","code":0,"data":["BTC.BTC","BTC.USDT","EOS.USDT","GMEX_CI_BTC","GMEX_CI_EOS"]}

// 然后服务器端就会每500毫秒批量推送一次，通常是二进制压缩后的数组消息，解压，json反序列，然后循环数组里的消息吧。。。

// 没有专门的取消指令，只要传个空数组即取消
{"req":"SubTicks","rid":"161","expires":1584625798179,"args":[]}
{"rid":"161","code":0,"data":null}

// 注意1: 订阅时参数是交易对名字列表，包括综合指数名字；和Sub时是不一样的，这里已经规定了是tick了。
// 注意2: 每次批量订阅都会替换上次的订阅主题, 订阅数最多256个。
...
为了节约网络流量，特别提供慢速模式，此时tick会1000毫秒推一次，order20会600毫秒推一次.
...
args: {
    ...
    "OType": 1,         // 报价类型, 1:Limit(限价委单), 2: Market(市价委单,匹配后转限价), 3: StopMarket (市价止损), 4:市价止损/盈利, 其它请参考下面OfferType定义;
    "Prz": 8000,        // 价格
...
## 关于 websocket.BinaryMessage 和'消息压缩'以及'数组消息'

尽管http层上的ws可以自己协商进行压缩减少传输带宽，还是有很多人觉得压缩后通过二进制方式传输更好。
于是我们也进行一下支持。在进行ws连接时，在url上增加参数'/zmod'即可：

```txt
启用二进制消息压缩的地址
行情服务： wss://api-market.gmex.io/v1/market/zmod
交易服务： wss://api-trade.gmex.io/v1/trade/zmod
```

注意在z模式下，服务器是有选择性的对消息进行压缩后通过二进制来传输，不少消息依旧使用 TextMessage 传输。
同时，此模式下，会启用数组消息，即，服务器会将多条消息，合并为数组打包发给客户端，客户端收到消息后，把
数组拆开，里面就是一条条的消息内容了，数组里消息的顺序是按发送时间顺序的。
