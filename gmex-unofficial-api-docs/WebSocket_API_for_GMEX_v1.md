...
{"rid":"2","code":0,"data":
    {
        "Sym":"EOS.USDT","Typ":"1m","Count":5,"InitSec":1573731480,
...
*重要说明*
如果你使用的是全仓模式，请忽略这两个调用，而且请不要使用他们。
如果你确实需要逐仓模式，请谨慎使用这两个调用。
一开始您没有仓位，通过 PosLeverage 来设置杠杆后，系统会自动为你造出一个Sz=0的对应仓位，
里面的杠杆信息即是这个交易对当前的杠杆，此时你下单成交后，仓位信息里的杠杆依旧是这个设置的值。
如果你的仓位里没有这个交易对的仓位信息，则说明该交易对默认是全仓模式。
另外，调整保证金是针对仓位的没，因此必须指定PId来进行操作。
